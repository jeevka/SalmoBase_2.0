...
def store_score(NScore,temp):
	if NScore.has_key(temp[1]):
		if float(NScore[temp[1]]) < float(temp[11].strip()):
			NScore[temp[1]] = float(temp[11].strip())
	else:
		NScore[temp[1]] = float(temp[11].strip())
...
def main_main(File_name,infile,qlength):
	...
	with open(File_name) as infile:
		for i in infile:
			...
			# Store the Score
			NScore = store_score(NScore,temp)
	...
	sorted_score = sorted(NScore, key=NScore.get)	
	...
	if len(sorted_cov) >=5:
	   ...
	   N = len(sorted_cov) 
	M1 = 0
	for i in xrange(len(sorted_cov)-1,len(sorted_cov)-N-1,-1):
	    try:
	    	ID = sorted_score[i]
	    	M = 0
	    	for j in RANGE[ID]:
